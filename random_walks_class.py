...
class RandomWalk:
    ...
    方法:
        _generate_displacement_vectors: 生成位移向量，用于append_steps和 append_runs方法
    ...
    def _generate_displacement_vectors(self,
                                       runs : int | np.integer,
                                       nsteps : int | np.integer) -> np.ndarray:
        """
        生成每次每步的位移向量。
        参数:
        ...
        返回:
            numpy数组: 形状(runs, nsteps, self.dim)
        """
        # 随机每次每步的移动维度和距离
        ...
        # 使用数组索引对位移向量数组的两个轴进行全遍历，并对维度轴索引，在指定位置替换为移动距离
        displacement_vectors = np.zeros((runs, nsteps, self.dim))  # 初始化位移向量
        displacement_vectors[walk_indices, step_indices,
                                dim_indices] = move_distances  # 位移向量
    ...
    def append_steps(self,
        ...
        参数:
            nsteps(int): 当前每次漫步的新增步数，必须为正整数，默认为 1
        返回:
            numpy数组: 形状为(self.pos.shape[0],
                            self.pos.shape[1] + nsteps,
                            self.dim)
        """
        # 参数检测
        validate_int(nsteps)
        ...
        # 生成位移向量，并根据当前位置对每次漫步的第一步位移向量进行修正
        displacement_vectors = self._generate_displacement_vectors(RUNS, nsteps)
        displacement_vectors[:, 0:1] =  (self.pos[:, -1:, :]
                                         + displacement_vectors[:, 0:1, :])
    ...
    def append_runs(self,
        ...
        参数:
            runs(int): 新增的漫步数量，必须为正整数，默认为 1
        返回:
            numpy数组: 形状为(self.pos.shape[0] + runs,
                            self.pos.shape[1],
                            self.dim)
        """
        # 参数检测
